...
function displaySlide()
{
  ...
  let videos = liveregion.querySelectorAll("video")
  if (videos.length > 0) {
  ...
  if (curslide.id && secondwindow)
    secondwindow.open("#" + curslide.id, "b6+ second window");
...
function createHelpText()
{
  ...
  iframe.src = 'data:text/html,' + encodeURIComponent(
    "<!DOCTYPE html>\n" +
      ...
      "<tr><td><kbd>2</kbd><td>open second window\n" +
...
function openSecondWindow()
{
  if (secondwindow == null || secondwindow.closed)
    secondwindow = open(location, "b6+ second window");
  secondwindow.focus();
}
